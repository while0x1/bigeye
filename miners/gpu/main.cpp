...
int main(int argc, char *argv[]) {
    printf("CLTUNA OpenCL core v2.0\n");
    ...
    /*
    std::string in_data = "d8799f508ed961efd99c2a5cc3de6d58de3ea1c65820f3c78d1a9f0ff7695bf7ef84156219684b32fc28bc54b7d683a10973efc38f751a03c5bb5e1947325820000000077c4d58113b1ea6bedc0807ab7591aec63549cc9be965dff2b1a1d36807199864ff";
    int j=0;
    for (unsigned int i = 0; i < in_data.length(); i += 2) {
      bytes[j++] = strtol(in_data.substr(i, 2).c_str(), NULL, 16);
    }

    sha256_crypt(bytes, j, 0, 6, 65535, result);
    if (result[0] != 0) {
        ...
        while(true) { 
            ...
            while( strlen(tmp_message) < 134) {
                ...
                if (strlen(tmp_message) < 134) {
                    printf("\x1b[31merror: invalid message. %ld \x1b[0m\n", strlen(tmp_message));
                    break;
                }
            }
            ...
            // HEX to bytes
            int msgsize = strlen(txbytes_hex)/2;
            tallymarker_hextobin(txbytes_hex, bytes, msgsize );
            bytes[msgsize]=0;
            ...
            uint64_t n_hashes = 0;
            bool found = false;
            while (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - time_1).count() < 1100) {
                memset(result, 0, 256);

                // Do the magic and get some fish!
                sha256_crypt(bytes, msgsize, 0, LZ, DN, result);
            ...
            if (hash_rate_average < 0) {
                ...
                hash_rate_average = 0.3*hash_rate + 0.7*hash_rate_average;
            }
            ...
            snprintf(server_message, 2000, ". %f", hash_rate_average);
            if (!found) {
                if (send(client_sock, server_message, strlen(server_message), 0) < 0){
                    printf("Can't send\n");
                    break;
