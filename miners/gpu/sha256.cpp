...
static size_t input_stride = 128;
...
void prepare_result(char* output, uint64_t plaintext_length, uint64_t j) {
    ...
    sprintf(output+plaintext_length*2, ":");
    for(int i=0; i<8; i++)
    {
        sprintf(output+plaintext_length*2+1+i*8, "%08x", partial_hashes[result_size*j+i]);
    }
}
...
void sha256_crypt(char* input, int string_len, int j_offset, int LZ, int DN, char* output)
{
    ...
    if (LZ == 2) {
        ...
        if (partial_hashes[result_size*j] < ((DN & 0xffff) << 8)) {
                prepare_result(output, string_len, j);
            }
        }
    // 000XXXX-
    } else if (LZ == 3) {
        for (int j=0;j<npar;j++) {
        if (partial_hashes[result_size*j] < ((DN & 0xffff) << 4)) {
                prepare_result(output, string_len, j);
            }
        }
    // 0000XXXX
        ...
        for (int j=0;j<npar;j++) {
        if (partial_hashes[result_size*j] < ((DN & 0xffff)) ) {
                prepare_result(output, string_len, j);
            }
        }
    // 00000XXX X-------
    ...
    if (j_offset > 0) {
        prepare_result(output, string_len, 0);
        printf("%s\n", output);
        prepare_result(output, string_len, 1);
        printf("%s\n", output);
        prepare_result(output, string_len, npar-2);
        printf("%s\n", output);
        prepare_result(output, string_len, npar-1);
        printf("%s\n", output);
    }
